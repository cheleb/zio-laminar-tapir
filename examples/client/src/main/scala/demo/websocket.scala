...
private val echoWebsocket: Var[Uri] = Var(
  Uri.unsafeParse("https://echo.websocket.org")
)

def websocket =
  // Hub to send messages to the WebSocket
  // Initially None, meaning not connected
  // When connected, it will hold a Hub[WebSocketFrame]
  ...
  div(
    ...
    children <-- hubVar.signal.map:
      case None =>
        openConnectionUI(hubVar)

      case Some(hub) =>
        connnectedUI(hubVar, hub)
  ).withSnippet:
    """|for
...
private def chooseServer(): Dropdown.Element =
  Dropdown(
    _.onSelect
      .map(
...
private def openConnectionUI(hubVar: Var[Option[Hub[WebSocketFrame]]]) =
  List(
    chooseServer(),
...
private def connnectedUI(
    hubVar: Var[Option[Hub[WebSocketFrame]]],
